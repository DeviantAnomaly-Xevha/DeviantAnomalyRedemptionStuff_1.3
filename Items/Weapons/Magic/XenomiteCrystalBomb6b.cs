...
namespace DeviantAnomalyRedemptionStuff.Items.Weapons.Magic
{
    ...
    public class XenomiteCrystalBomb6b : ModItem
    {
        public float Charge;
        ...
        public override void HoldItem(Player player)
        {
            ...
            if (!player.channel && Charge >= 30f)//if the use button is released after charging for at least 30 frames
            {
                ...
                else if (player.statMana >= item.mana)
                {
                    ...
                    {
                        int a = Projectile.NewProjectile(player.Center.X, player.Center.Y, (AimDirection.X * item.shootSpeed) * .625f, (AimDirection.Y - 11f), ModContent.ProjectileType<X2XenomiteCrystalBombCharged_Proj>(), item.damage * 3, 0, item.owner);
        ...
        public override void AddRecipes()
        {
            ...
            if (RedeMod != null)
            {
                recipe.AddRecipeGroup("DeviantAnomalyRedemptionStuff:tier 5/red Xenomite Crystal Bomb weapon");
